...
const KEY_COUNT = matrix_scanner.KEY_COUNT;
...
pub const Debouncer = struct {
    ...
    pub fn init(scanner: *MatrixScanner) Debouncer {
        return Debouncer{
            .scanner = scanner,
    ...
    pub fn process(self: *Debouncer) !EventQueue {
        var events = EventQueue{};

        // Scan the matrix
        const pressed_keys = try self.scanner.scan();

        // Update debounce state for each key
        for (0..KEY_COUNT) |key_idx| {
            const row: u8 = @intCast(key_idx / matrix_scanner.COLS);
            const col: u8 = @intCast(key_idx % matrix_scanner.COLS);
    ...
    pub fn isPressed(self: *const Debouncer, key_idx: u8) bool {
        if (key_idx >= KEY_COUNT) return false;
        return self.states[key_idx].is_pressed;
    }
};

// Need microzig import for Key type
